...
def save_depth_npy(depth: np.ndarray, output_path: str) -> None:
    """Save raw depth values to a NumPy file."""
...
def save_depth_vis(depth: np.ndarray, output_path: str) -> None:
    ...
    if valid_count == 0:
        ...
        # Create black images as fallback
        ...
        Image.fromarray(np.zeros(depth.shape, dtype=np.uint8), mode="L").save(gray_path)
        Image.fromarray(np.zeros(depth.shape, dtype=np.uint8), mode="L").save(vis_path)
        Image.fromarray(np.zeros(depth.shape, dtype=np.uint8), mode="L").save(mask_path)
        return
